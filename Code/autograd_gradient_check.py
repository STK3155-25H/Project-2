...
# === CONFIG: directory centralizzate =================================
# Assumo che config.py sia accanto a questo file o importabile nel PYTHONPATH
from config import get_output_subdir, BENCHMARK_OUTPUT_DIR

# Sotto-cartella dedicata a questi test
AUTOGRAD_CHECKS_DIR = get_output_subdir("autograd_grad_checks")

# Utility per comporre path file nella cartella di lavoro
def _make_path(filename: str) -> str:
    return os.path.join(AUTOGRAD_CHECKS_DIR, filename)
...
# ================================================================
# Utilities di confronto + I/O CSV
# ================================================================
...
def _append_csv_row(path: str, header: list, row: list):
...
#    -> salva in cost_grad_checks.csv
# ================================================================
def run_cost_grad_checks(seed=314, atol=1e-8, rtol=1e-6, save=True):
    rng = onp.random.default_rng(seed)
    print("\n=== COST GRADIENT CHECKS (∂L/∂X) ===")

    csv_path = _make_path("cost_grad_checks.csv")
    header = ["timestamp", "seed", "name", "max_abs_diff", "rel_err", "passed", "atol", "rtol", "N_or_shape"]
    ...
    passed, max_abs, rel = compare_arrays(dL_dX_anal, dL_dX_auto, name="CostOLS", atol=atol, rtol=rtol)

    if save:
        _append_csv_row(
            csv_path, header,
            [datetime.utcnow().isoformat(), seed, "CostOLS", f"{max_abs:.6e}", f"{rel:.6e}", passed, atol, rtol, str(X.shape)]
        )
    ...
    passed, max_abs, rel = compare_arrays(dL_dX_anal, dL_dX_auto, name="CostLogReg", atol=atol, rtol=rtol)

    if save:
        _append_csv_row(
            csv_path, header,
            [datetime.utcnow().isoformat(), seed, "CostLogReg", f"{max_abs:.6e}", f"{rel:.6e}", passed, atol, rtol, str(Xp.shape)]
        )
    ...
    passed, max_abs, rel = compare_arrays(dL_dX_anal, dL_dX_auto, name="CostCrossEntropy", atol=atol, rtol=rtol)

    if save:
        _append_csv_row(
            csv_path, header,
            [datetime.utcnow().isoformat(), seed, "CostCrossEntropy", f"{max_abs:.6e}", f"{rel:.6e}", passed, atol, rtol, str(Xs.shape)]
        )

# ================================================================
# 2) Test: derivate delle activation vs Autograd (elementwise)
#    -> salva in activation_deriv_checks.csv
# ================================================================
def run_activation_deriv_checks(seed=2718, atol=1e-8, rtol=1e-6, save=True):
    rng = onp.random.default_rng(seed)
    print("\n=== ACTIVATION DERIVATIVE CHECKS (element-wise) ===")

    csv_path = _make_path("activation_deriv_checks.csv")
    header = ["timestamp", "seed", "activation", "max_abs_diff", "rel_err", "passed", "atol", "rtol", "N"]
    ...
    for act in act_list:
        ...
        passed, max_abs, rel = compare_arrays(d_anal, d_auto, name=f"d{act.__name__}", atol=atol, rtol=rtol)
        if save:
            _append_csv_row(
                csv_path, header,
                [datetime.utcnow().isoformat(), seed, act.__name__, f"{max_abs:.6e}", f"{rel:.6e}", passed, atol, rtol, X.size]
            )

# ================================================================
# 3) Rete: forward/backward + gradient check
#    -> salva in network_gradient_check.csv (una riga riassuntiva)
# ================================================================
...
def gradient_check(d_in=1, d_hidden=10, d_out=1, lam_l2=1e-4, n_samples=100, seed=314,
    ...
    passed, max_abs, rel = compare_arrays(grad_manual, onp.asarray(grad_auto),
    ...
    if save:
        csv_path = _make_path("network_gradient_check.csv")
        header = [
            "timestamp", "seed", "d_in", "d_hidden", "d_out", "lam_l2",
            "n_samples", "max_abs_diff", "rel_err", "passed", "atol", "rtol", "idx_worst"
        ]
        _append_csv_row(
            csv_path, header,
            [datetime.utcnow().isoformat(), seed, d_in, d_hidden, d_out, lam_l2,
             n_samples, f"{max_abs:.6e}", f"{rel:.6e}", passed, atol, rtol, idx_worst]
        )
...
if __name__ == "__main__":
    ...
    # I check scrivono i risultati in:
    #   output/autograd_grad_checks/
    # (cartella creata automaticamente via config.get_output_subdir)
    run_cost_grad_checks(seed=314, atol=ATOL, rtol=RTOL, save=True)
    run_activation_deriv_checks(seed=2718, atol=ATOL, rtol=RTOL, save=True)
    gradient_check(d_in=1, d_hidden=10, d_out=1, lam_l2=1e-4, n_samples=100, seed=314,
                   atol=ATOL, rtol=RTOL, save=True)
